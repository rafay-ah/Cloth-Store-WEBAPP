...
@foreach($collection as $item)
        ...
        <div class="row">
            <div class="col-lg-12 text-center pt-2">
                <h2 style="font-weight: 800; font-family: sans-serif;">Store</h2>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-4 col-md-6 col-sm-12 p2-5 text-center">
                <img class="shadow-lg" src="{{asset('uploads')}}/{{$item->image}}" alt="" style="width: 30vh;">
            </div>
            <div class="col-lg-4 col-md-6 col-sm-12 p-4">
                <div class="form-group" style="font-weight: 700;">
                        <label for="name ">Store Name:{{$item->Store_Name}}</label>
                        <hr>
                        <label for="mail">Store Code: {{$item->Store_Code}}</label>
                        <hr>
                        <label for="psd">Email: {{$item->Email}}</label>
                        <hr>
                        <label for="">Password:{{$item->password}}</label>
                        <hr>
                </div>
            </div>
            <div class="col-lg-4 col-md-6 col-sm-12 p-4">
                <div class="form-group" style="font-weight: 700;">
                        <label for="pno">Seller Name:{{$item->Seller_Name}}</label>
                        <hr>
                        <label for="">Phone No:{{$item->Phone}}</label>
                        <hr>
                </div>
                <a href={{"http://127.0.0.1:8000/admin/DeleteStore/$item->id"}}>  <button type="button" class="btn btn-dark btn-block shadow-lg" style="color: white; margin-top: 10vh;">Delete</button></a>
            </div>
        </div>
    </div>
    @endforeach
        ...
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 text-center p-4">
                <h4>Service Not Available! <a href={{"/admin/Addstore"}}>Add New</a></h4>
            </div>
        </div>
        @endif
    </div>
